...
export default function RSVPCard() {
  ...
  const submit = () => {
    ...
    window.location.href = `https://wa.me/${WP}?text=${msg}`
    setSubmitted(true)
  }
  ...
  if (submitted) return (
    <div className="ty-card">
      ...
      <p className="ty-names">Əli &amp; Aynur</p>
      <p className="ty-date">3 Oktyabr 2026</p>
    </div>
  )

  return (
    <div className="rsvp-card">
      <p className="rsvp-note">Zəhmət olmasa əvvəlcədən cavab verin</p>

      <div className="rsvp-f">
        <label className="rsvp-lbl">Ad Soyad *</label>
        <input
          className={`rsvp-inp${nameErr ? ' err' : ''}`}
          type="text"
      ...
      <div className="rsvp-f">
        ...
        <div className="radio-row">
          <button className={`radio-btn${form.attending === 'yes' ? ' sel' : ''}`} onClick={() => set('attending', 'yes')}>
            ✓ Bəli, iştirak edəcəm
          </button>
          <button className={`radio-btn${form.attending === 'no' ? ' sel' : ''}`} onClick={() => set('attending', 'no')}>
      ...
      <div className="rsvp-f">
        ...
        <textarea
          className="rsvp-ta"
          rows={3}
          placeholder="Gəlin-bəyə xoş arzularınız..."
          value={form.message}
